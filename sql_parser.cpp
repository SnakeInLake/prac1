...
bool executeInsert(const string& sql_query) {
    ...
    if (fs::exists(csv_filename)) {
        ...
        output_csv << table_name << "_pk,";  // Столбец первичного ключа
    ...
    // Вставляем порядковый номер строки в первый столбец
    int new_pk = total_row_count + 1;  // Порядковый номер строки
    output_csv << new_pk << ","; 
    ...
    // --- Обновляем файл _pk_sequence ---
    string pk_sequence_filename = base_csv_filename + table_name + "_pk_sequence";
    ofstream pk_file(pk_sequence_filename);
    if (!pk_file.is_open()) {
        cerr << "Ошибка: не удалось открыть файл для записи " << pk_sequence_filename << endl;
        return false;
    }

    // Записываем последнее значение первичного ключа
    pk_file << new_pk << endl;
    pk_file.close();

    cout << "Файл " << pk_sequence_filename << " успешно обновлён с новым значением: " << new_pk << endl;
...
while (current_row_left != nullptr) {
    ...
    string left_value_column = left_value.substr(left_value.find('.') + 1); 
        ...
        if (condition_result) {
            cout << "Строка из левой таблицы: " << current_row_left->data << endl;
            cout << "Строка из правой таблицы: " << current_row_right->data << endl;
        }
